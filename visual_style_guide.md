...
**推荐作为唯一真源（source of truth）**：`src/disaster/plot_style.py`  
...
> 说明：该策略已在 `src/disaster/plot_style.py::_resolve_times_font()` 与 `paper_rcparams()` 中实现。
...
统一使用 Okabe–Ito 调色板（`src/disaster/plot_style.py:OKABE_ITO`）：
...
- 统一用 `save_figure(fig, path)` 保存（见 `src/disaster/plot_style.py`）。
