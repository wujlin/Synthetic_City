#!/usr/bin/env bash
# PI experiments on the paper-consistent 5-var coarse setting (K=32):
# 1) output examples
# 2) convergence (save_epochs + checkpoint evaluation)
# 3) MC stability
#
# 注意：
# - 本脚本不使用 set -e，报错后不会自动退出，便于定位问题。
# - 该脚本固定使用 K=32 joint 表，避免与 K=512 结果混用。
...
JOINT_K32_DIR="${JOINT_K32_DIR:-$OUT_ROOT/_us_puma_5var_joint_k32_20260220T172157Z}"
JOINT_K32="${JOINT_K32:-$JOINT_K32_DIR/puma_5var_joint_wide.csv}"
...
RUN_EXP2="${RUN_EXP2:-$OUT_ROOT/_exp2_convergence_train_k32_${TS}}"
RUN_EXP23="${RUN_EXP23:-$OUT_ROOT/_exp23_eval_k32_${TS}}"
FIG_DIR="${FIG_DIR:-Essay/figures}"
mkdir -p "$RUN_EXP2" "$RUN_EXP23" "$FIG_DIR"

echo ">>> Input check"
ls -lh "$JOINT_K32" "$PUMA_ZIP" "$US_HET_JSON" "$MI_HET_JSON"

echo ">>> [Exp2-Train-K32] pairwise model with save_epochs"
python -u tools/train_us_puma_5var_diffusion.py \
  --joint_wide_csv "$JOINT_K32" \
  ...
  --save_epochs "100,200,500,1000,2000,3000,5000,7000,10000,20000,30000" \
  --save_final_model \
...
echo ">>> [Exp2-Train-K32] RC=${PIPESTATUS[0]}"

CKPT_ROOT="$RUN_EXP2/checkpoints/pairwise/leave_mi_out"
FINAL_CKPT="$CKPT_ROOT/final.pt"

echo ">>> [Exp2-Eval-K32] convergence curve from saved checkpoints"
...
echo ">>> [Exp2-Eval-K32] RC=${PIPESTATUS[0]}"

echo ">>> [Exp3-K32] MC stability"
...
echo ">>> [Exp3-K32] RC=${PIPESTATUS[0]}"

echo ">>> [Exp1-K32] output examples"
...
echo ">>> Build fig_verification (a,b,c) for K=32"
...
echo ">>> [fig_verification-K32] RC=${PIPESTATUS[0]}"
...
ls -lh \
  ...
  "$FIG_DIR/fig_03_output_examples.png" \
