...
WIDE_CSV="${WIDE_CSV:-$DATA_ROOT/us/processed/external_c2f/extc2f_full_income_v2_teacher_pums_2023_puma_us_wide.csv}"
SCHEMA_JSON="${SCHEMA_JSON:-$DATA_ROOT/us/processed/external_c2f/extc2f_full_income_v2_teacher_pums_2023_puma_us.schema.json}"
...
INCOME_REGIME_WEIGHT="${INCOME_REGIME_WEIGHT:-0.0}"
INCOME_REGIME_CONSISTENCY_WEIGHT="${INCOME_REGIME_CONSISTENCY_WEIGHT:-0.0}"
AUX_T_GATE="${AUX_T_GATE:--1}"
PREDICT_MODE="${PREDICT_MODE:-diffusion}"
BLEND_ALPHA="${BLEND_ALPHA:-0.0}"
REGIME_PROJECTION_ALPHA="${REGIME_PROJECTION_ALPHA:-0.0}"
...
RUN_DIR="${RUN_DIR:-outputs/_us_puma_external_c2f_full_income_v2_teacher_$(date -u +%Y%m%dT%H%M%SZ)}"
...
CMD=(
  ...
  --income_regime_weight "$INCOME_REGIME_WEIGHT"
  --income_regime_consistency_weight "$INCOME_REGIME_CONSISTENCY_WEIGHT"
  --aux_t_gate "$AUX_T_GATE"
  --predict_mode "$PREDICT_MODE"
  --blend_alpha "$BLEND_ALPHA"
  --regime_projection_alpha "$REGIME_PROJECTION_ALPHA"
