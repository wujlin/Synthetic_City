...
RUN_DIR="${RUN_DIR:-outputs/_us_puma_external_joint_hier_full_${FINE_INPUT_MODE}_$(date -u +%Y%m%dT%H%M%SZ)}"
LR="${LR:-1e-3}"
...
"$PYTHON_BIN" -u tools/model/train_external_joint_hier_full.py \
  ...
  --seed "$SEED" \
  --save_final_model \
  --run_label "external_joint_hier_full" \
  --out_dir "$RUN_DIR"
