...
"""
Shared-latent hierarchical trainer for the 5-variable full external schema:

  AGEP_fine(10) x SEX(2) x SCHL_fine(5) x ESR_fine(5) x EARN_fine(6) = K=3000

The coarse head intentionally remains the validated 4-attribute lite schema:

  AGEP_lite(4) x SEX(2) x SCHL_lite(3) x ESR_lite(3) = K=72

Rationale:
- This is a diagnostic pilot of whether the shared regional context can support
  the full 5-way joint directly.
- We do not introduce a hand-crafted earnings coarse hierarchy here, so any gain
  is attributable to the shared latent route rather than a new earnings-side
  aggregation design.
"""
...
base.COARSE_VARIABLE_ORDER = ["AGEP_bin", "SEX", "SCHL_allpop", "ESR_allpop"]
        ...
        for si, _ in enumerate(base.FINE_CATEGORIES["SEX"]):
            for schl_lab in base.FINE_CATEGORIES["SCHL_allpop"]:
                ...
                for esr_lab in base.FINE_CATEGORIES["ESR_allpop"]:
                    ...
                    for _wi, _ in enumerate(base.FINE_CATEGORIES["EARN_16p_bin"]):
                        out[k, kc] = 1.0
                        k += 1
    return out


base._build_fine_to_coarse_matrix = _build_fine_to_coarse_matrix_full_earn
