...
BUILD_TARGET_ONCE="${BUILD_TARGET_ONCE:-1}"
...
DEVICE="${DEVICE:-cuda}"
SEED="${SEED:-0}"
RUN_DIR="${RUN_DIR:-outputs/_us_puma_external_earn_from_context_$(date -u +%Y%m%dT%H%M%SZ)}"
...
if [[ "$BUILD_TARGET_ONCE" == "1" ]]; then
  "$PYTHON_BIN" -u tools/data/build_external_target_earn_v1_us.py \
    --all_states \
    --pums_year "$PUMS_YEAR" \
    --pums_dir "$PUMS_DIR" \
...
"$PYTHON_BIN" -u tools/model/train_external_earn_from_context.py \
