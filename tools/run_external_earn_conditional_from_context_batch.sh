...
DEVICE="${DEVICE:-cuda}"
SEEDS="${SEEDS:-0 1 2}"
RUN_PREFIX="${RUN_PREFIX:-_us_puma_external_earn_conditional_batch}"
...
if [[ "$BUILD_TARGET_ONCE" == "1" ]]; then
  "$PYTHON_BIN" -u tools/build_external_target_earn_conditional_v1_us.py \
    --all_states \
    --pums_year "$PUMS_YEAR" \
    --pums_dir "$PUMS_DIR" \
...
for seed in "${SEED_ARR[@]}"; do
  run_dir="outputs/${RUN_PREFIX}_${CONDITION_MODE}_seed${seed}"
  RUN_DIRS+=("$run_dir")
  ...
  bash tools/run_external_earn_conditional_from_context.sh
done

SUMMARY_OUT="outputs/${RUN_PREFIX}_summary"
"$PYTHON_BIN" -u tools/summarize_external_earn_conditional_runs.py \
  --label external_earn_conditional_batch \
  --run_dirs "${RUN_DIRS[@]}" \
  --out_dir "$SUMMARY_OUT"
