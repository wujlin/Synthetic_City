...
┌────────────────────────────────────────────────────────────┐
│  Layer 1: BG 级别基本人口骨架                               │
│  方法: DHC + IPF/优化 (或复用导师数据 + race 补充)          │
│  变量: age, sex, race, household_type                      │
│  状态: 可立即实施                                           │
│  长期角色: Scheme C 中的 PUMS person 属性来源之一            │
├────────────────────────────────────────────────────────────┤
│  Layer 2: 属性扩展 (income, education, employment)          │
│  方法: 条件扩散，训练于 PUMS                                │
│  条件: (age, sex, race) + 空间特征                          │
│  状态: 当前实验在做的 (Scheme B 的核心)                      │
│  长期角色: Scheme C 中 z_person 编码的输入                   │
├────────────────────────────────────────────────────────────┤
│  Layer 3: Person-Building 联合生成 (Scheme C 核心)          │
│  方法: 多源对齐 + 联合扩散                                  │
│  桥接: Veraset device 行为数据                              │
│  状态: ⚠️ 等 Veraset 数据到位后实施                         │
│  长期角色: 替代 Scheme B 的规则分配，实现数据驱动锚定        │
└────────────────────────────────────────────────────────────┘